...
public class PageResultDTO<DTO,EN> {   //JPQL의 결과로 나오는 Object []를 DTO로 변환
    //Object[] 내용: Board, writer(Member), replyCount(Long) >>   Board DTO 로
    ...
    public PageResultDTO(Page<EN> result, Function<EN,DTO> fn) {  //fn을 result에 적용 ( fn에는 entity> DTO 변환 함수
                                         //Function<파라미터 타입, 리턴 타입>
        //result: JPQL 결과인 Page<Object[]> 타입 객체.

       // Page의 각 요소 Object[] 안에는   slect에 위치한 Board,writer(Member) count(r) [Long]  을 각각 저장
                dtoList=result.stream().map(fn).collect(Collectors.toList());//map 함수로 fn 함수를 페이지의 모든 요소에 적용.
        //                 각 요소에 map 적용 후   collect 통해 모은 뒤 리스트로.
